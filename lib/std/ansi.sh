...
ansi.enable.powerline() {
  ...
  POWERLINE_HEART=$'\u2764'
  POWERLINE_STAR=$'\u2b50'
  POWERLINE_SAXOPHONE=$'\U1f3b7'
  POWERLINE_THUMBS_UP=$'\U1f44d'
}
...
ansi.disable.powerline() {
  ...
  POWERLINE_HEART="<3"
  POWERLINE_STAR="*"
  POWERLINE_SAXOPHONE="(YEAH)"
  POWERLINE_THUMBS_UP="(OK)"
}

ansi.enable() {
  [[ -n "${BASHLET_ANSI_FORCE_DISABLE-}" ]] && return 0
  ansi.Color.IsAvailable && ansi.enable.color && ansi.enable.style ||true
  ansi.Powerline.IsAvailable && ansi.enable.powerline ||true
}
